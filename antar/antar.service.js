...
export class AntarService {
    ...
    async notifyT2SSubscriber(payload, t2sPayload, card_payment_id, awsRequestId) {
        ...
        let logMetadata = {
            ...
            location: 'AntarService ~ notifyT2SSubscriber',
            awsRequestId
        };
    ...
    async notifyT2SSubscriberDirect(payload, t2sPayload, card_payment_id, awsRequestId) {
        let logMetadata = {
            orderId: payload.order_id,
            location: 'AntarService ~ notifyT2SSubscriberDirect',
            awsRequestId
        };
    ...
    async tokeniseCard(payload) {
        try {
            ...
            let queueUrl = process.env.QUEUE_URL_MASTERTOKEN;
            ...
            if (process.env.IS_OFFLINE) {
                ...
                queueUrl = process.env.LOCAL_QUEUE_URL_MASTERTOKEN;
            }
            ...
            console.log('Queue Url SQS for Master token: ', queueUrl);
            const params = {
                MessageBody: objectStringified,
                QueueUrl: queueUrl
            };
