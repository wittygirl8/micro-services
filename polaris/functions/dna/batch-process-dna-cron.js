...
export const processBatch = async (event, context) => {
    ...
    try {
        ...
        let cronResult = await cronHelper.start(
            {
                script: 'batch-process-dna-cron.js',
        ...
        var countQuery = `SELECT count(1) AS 'count' FROM customers a,payout_batch b WHERE a.id = b.customer_id AND a.country_id = 1 AND b.status='PENDING' AND a.business_name NOT LIKE'%test%' and payout_provider = 'DNA_BARCLAYS'`;
        ...
        var publishersMid = '';
        while (countResult != 0) {
            var q = `SELECT batch.*, other_customer_details.accountnumber, other_customer_details.sortcode, other_customer_details.bankname, other_customer_details.accountholder FROM payout_batch batch, other_customer_details,  customers where  customers.id = batch.customer_id and other_customer_details.customers_id = batch.customer_id  and  batch.status='PENDING' AND customers.country_id = 1 AND customers.business_name not like '%test%' ORDER by batch.batch_id LIMIT ${pageSize} OFFSET ${fetchedRows}`;
            ...
            var finalRecords = await batchInformation.filter((e) => {
                if (e.accountnumber && e.accountnumber.length == 8 && e.sortcode && e.sortcode.length == 6) {
                    var total = Number(String(e.total).replace('-', ''));
        ...
        console.log(`recordIDs`, recordIDs, 'recordIDsLength:', recordIDs.length);
        if (recordIDs.length > 0) {
            var updateQuery = `update payout_batch a, payout_batch_item b, payments c set a.status = 'COMPLETE', c.transaction_status_id = 5, c.withdrawn_status = 1 WHERE a.batch_id = b.batch_id and b.card_payment_id = c.id and a.batch_id in (${recordIDs})`;
            var countOfAffectedRecords = await sequelize.query(updateQuery, {
                type: sequelize.QueryTypes.UPDATE
            });
            console.log(`countOfAffectedRecords:${countOfAffectedRecords}`);
        }
