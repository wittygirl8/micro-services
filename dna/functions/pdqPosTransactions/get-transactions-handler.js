...
export const getTransactions = async (event, context) => {
    ...
    try {
        ...
        const merchants = await PartnerMerchant.findAll({ order: [['last_updated_at', 'DESC']] });
        ...
        const processedMerchants = merchants.map(({ merchant_id, partner_merchant_id, last_updated_at }) => ({
            merchant_id,
            partner_merchant_id,
            last_updated_at
        }));
...
const processMerchants = async (sqsClient, token, merchants, queueUrl) => {
    try {
        let lastDate = merchants.last_updated_at
            ? DateTime.fromJSDate(merchants.last_updated_at).toUTC()
            : DateTime.fromJSDate().toUTC().minus({ days: 3 });

        let fromDate = lastDate.startOf('minute').toISO();

        let toDate = DateTime.utc().endOf('minute').toISO();

        const periodAsDays = DateTime.utc().diff(lastDate, 'days').days;

        if (periodAsDays > 3) {
            console.debug(`Processing is greater than 3 days - ${lastDate.toISO()}`);
            toDate = lastDate.plus({ days: 3 });
        }
