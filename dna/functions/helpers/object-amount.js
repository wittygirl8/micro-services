...
export const amountObject = async (dbInstance, obj) => {
    console.log('merchant_id', obj.merchantId);

    //get country
    const customerDetails = await dbInstance.Customer.findOne({
    ...
    if (!customerDetails) {
        throw { message: 'Invalid merchantId' };
    }

    const { fee_tier_id, country_id } = customerDetails;
