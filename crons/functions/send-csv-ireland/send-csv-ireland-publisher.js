...
export const sendCsvIreland = async (event, context) => {
    const db = connectDB(
        process.env.DB_HOST,
    ...
    let logMetadata = {
        location: 'MessagingService ~ sendCSVIrelandPublisher',
    ...
    try {
        ...
        let cronResult = await cronHelper.start(
            {
                script: 'cron-csv-ireland-publisher.js',
        ...
        var countQuery = `SELECT count(*) as 'count' FROM batch LEFT JOIN other_customer_details ON other_customer_details.customers_id = batch.customer_id LEFT JOIN customers ON customers.id = batch.customer_id WHERE length(accountnumber) = 22 AND batch.status='PENDING' AND customers.country_id = 2  AND batch.batch_id > 0 ORDER by batch.batch_id`;
        ...
        if (countResult == 0) {
            await cronHelper.finish(
                {
                    output: `No pending transactions. <br>`,
        ...
        var publishersMid = '';
        while (countResult != 0) {
            var q = `SELECT batch.*, other_customer_details.accountnumber, other_customer_details.sortcode, other_customer_details.bankname, other_customer_details.accountholder, other_customer_details.bank_address_1, other_customer_details.bank_address_2, customers.customers_number, customers.customers_street, customers.customers_city FROM batch LEFT JOIN other_customer_details ON other_customer_details.customers_id = batch.customer_id LEFT JOIN customers ON customers.id = batch.customer_id WHERE length(accountnumber) = 22 AND batch.status='PENDING' AND customers.country_id = 2  AND batch.batch_id > 0 ORDER by batch.batch_id LIMIT ${pageSize} OFFSET ${fetchedRows}`;
            ...
            var finalRecords = await batchInformation.filter((e) => {
                if (e.accountnumber && e.accountnumber.length == 22) {
