...
test('[sendCSVUK] all data valid', async () => {
    const { CsvUKService } = require('../consumer/send-csv-uk.service');
    const csvukService = new CsvUKService();
    ...
    const result = await csvukService.init(event, { awsRequestId: 1 });
    expect(result.success).toBe(true);
});

test('[sendCSVUK] invalid data', async () => {
    const { CsvUKService } = require('../consumer/send-csv-uk.service');
    const csvukService = new CsvUKService();
    ...
    expect(async () => await csvukService.init(event, { awsRequestId: 1 })).rejects.toThrow(
